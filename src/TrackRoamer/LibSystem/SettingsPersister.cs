...
namespace TrackRoamer.Robotics.Utility.LibSystem
{
	...
	public class SettingsPersister
	{
		...
		public static bool restoreOrDefaultPortSettings()
		{
			...
			try
			{
                using (fs = new FileStream(controllerPortFilePath, FileMode.Open, FileAccess.Read, FileShare.Read))
                {
		...
		public static void savePortSettings()
		{
			...
			{
				string controllerPortFilePath = Project.GetMiscPath(Project.CONTROLLER_PORTCONFIG_FILE_NAME + "-" + currentControllerMake + ".xml");
                using (fs = new FileStream(controllerPortFilePath, FileMode.Create, FileAccess.Write, FileShare.ReadWrite))
                {
