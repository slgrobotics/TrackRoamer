...
namespace TrackRoamer.Robotics.LibMapping
{
    ...
    public class MapperVicinity : List<IDetectedObject>
    {
        ...
        private Direction _robotDirection = new Direction() { heading = 0.0d };       // robot orientation relative to true North
        ...
        // this is for rare publuc use - you must lock the MapperVicinity instance while using this list:
        public List<IDetectedObject> detectedObjects { get { return this; } } // = new List<IDetectedObject>();
        ...
        public void computeMapPositions()
        {
            lock (this)
            {
                try
                {
                    ...
                    {
                        Direction dirToObj = new Direction() { bearing = robotPosition.bearing(idobj.geoPosition) - robotDirection.heading };

                        Distance distToObj = idobj.geoPosition.distanceFrom(robotPosition);

                        idobj.relPosition = new RelPosition(dirToObj, distToObj);
                        ...
                        if (idobj.objectType != DetectedObjectType.Mark)    // "Mark" objects are not participating in obstacle or cell calculations
                        {
                            ...
                            if (relCell != null)
                            {
                                relCell.detectedObjects.Add(idobj);
                                idobj.cellCounter++;
                            }
                            ...
                            if (geoCell != null)
                            {
                                geoCell.detectedObjects.Add(idobj);
                                idobj.cellCounter++;
                            }
                        }
                    }
